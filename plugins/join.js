...
let handler = async (m, { conn, usedPrefix, text, isOwner }) => {
  ...
  if(isOwner || m.fromMe) {
    ...
    let data = global.owner.filter(([id, _, isCreator]) => id && isCreator).map(v => v[0])
    for(let jid of data) conn.reply(jid + "@s.whatsapp.com" , `request to join group from ${conn.getName(m.sender)}\n\n${text}`, m)
