...
let handler = async (m, { conn, usedPrefix: _p, __dirname }) => {
  try {
    ...
    // Offset    0 is  0.00
    // Offset  420 is  7.00
    let weton = ["Pahing", "Pon", "Wage", "Kliwon", "Legi"][Math.floor(Number(tz) / 84600000) % 5]
    let week = d.toLocaleDateString(locale, { weekday: "long" })
    let date = tz.year() + "-" + String(tz.month() + 1).padStart(2, "0") + "-" + String(tz.date()).padStart(2, "0")
    let dateIslamic = Intl.DateTimeFormat(locale + "-TN-u-ca-islamic", {
      day: "numeric",
      month: "long",
      year: "numeric"
    }).format(d)
    let time = String(tz.hours()).padStart(2, "0") + ":" + String(tz.minutes()).padStart(2, "0") + ":" + String(tz.seconds()).padStart(2, "0")
